...
:
    ...
    T0_("T0", TSat().dimensions(),0.0),// Temperature criteria
   	Cm1_("Cm1_", 2.0*Cv_*hEvap_*rho2_/((2.0-Cv_)*pow(2.0*M_PI*R_,0.5)))
{
//    correct();
}
...
Foam::phaseChangeTwoPhaseMixtures::Hardt::mDotAlphal() //const
{
    ...
    (
    	-rc_*Cm1_*min(T - TSat_ ,T0_)*AbyV()/sqrt(pow(TSat_,3.0))
    	//*neg(T - TSatLocal() + 1E-05*TSat())
    	,
        -rv_*Cm1_*max(T - TSat_ ,T0_)*AbyV()/sqrt(pow(TSat_,3.0))
